...
def describe_app_version(
    version_label: str, eb_app_name: str
) -> ApplicationVersionDescriptionTypeDef | None:
    """
    ...
    found_app_versions: list[ApplicationVersionDescriptionTypeDef] = (
        search_app_version_response["ApplicationVersions"]
    )

    if len(found_app_versions) > 1:
        raise Exception(
            f"Unexpected number of version ({len(found_app_versions)} > 1) matching label {version_label} in application {eb_app_name}."
        )
...
def eb_app_version_exists(version_label: str, eb_app_name: str) -> bool:
    ...
    ## Search Application version by label
    app_version = describe_app_version(
        version_label=version_label, eb_app_name=eb_app_name
    )
...
def get_eb_app_version_status(version_label: str, eb_app_name: str) -> str:
    ...
    if app_version is None:
        raise Exception(
            f"No application version found with label {version_label} in application {eb_app_name}"
        )
    else:
...
def create_eb_app_version(
    version_label: str,
    eb_app_name: str,
    source_bundle: S3LocationTypeDef,
    ...
    application_version_status: str = create_app_version_response["ApplicationVersion"][
        "Status"
    ]
    ...
    walltime = 0
    while (
        application_version_status in ["processing", "building"] and walltime < TIMEOUT
    ):
        sleep(INTERVAL)
        walltime += INTERVAL

        application_version_status = get_eb_app_version_status(
            eb_app_name=eb_app_name, version_label=version_label
        )

    if walltime >= TIMEOUT:
        raise Exception(
            "EB app version failed to report creation completion before timeout."
        )
        ...
        raise Exception(
            f'Unexpected EB app version status "{application_version_status}" reported.'
        )
    else:
        return describe_app_version(
            version_label=version_label, eb_app_name=eb_app_name
        )
