...
def describe_eb_environment(
    environment_name: str,
) -> EnvironmentDescriptionTypeDef | None:
    """
    ...
    found_environments: list[EnvironmentDescriptionTypeDef] = (
        search_environment_response["Environments"]
    )

    if len(found_environments) > 1:
        raise Exception(
            f"Unexpected number of environments ({len(found_environments)} > 1) matching name {environment_name}."
        )
...
def get_eb_environment_health(environment_name: str) -> EBEnvironmentHealth:
    ...
    ## Search Application version by label
    environment: EnvironmentDescriptionTypeDef | None = describe_eb_environment(
    ...
    if environment is None:
        ...
        return {"Color": health_color, "Status": health_status}
