...
app = App()
stack = SharedInfraStack(app, "pytest-stack", env=agr_aws_environment)
...
def test_health_notifications_sns() -> None:
    ...
    sns_topic = template.find_resources(
        type=ResourceType.SNS_TOPIC.compliance_resource_type,
        props={"Properties": {"TopicName": "pavi-health-notifications"}},
    )
    ...
    template.has_resource(
        ...
        props={"Properties": {"SnsTopicArns": [{"Ref": list(sns_topic.keys()).pop()}]}},
    )
