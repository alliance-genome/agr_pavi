...
class PaviExecutionEnvironment:
    ...
    def __init__(
        ...
        scope: Stack,
        env_suffix: str,
        shared_logs_group: Optional[str],
        shared_work_dir_bucket: Optional[str],
    ) -> None:
        """
        ...
        if not shared_work_dir_bucket:
            ...
            self.nf_output_bucket = s3.Bucket(
                scope=scope,
                id="pavi-pipeline-nf-workdir-bucket",
        ...
        s3_bucket_access_statements = [
            ...
            iam.PolicyStatement(
                ...
                resources=[
                    self.nf_output_bucket.bucket_arn,
                    self.nf_output_bucket.bucket_arn + "/*",
                ],
            ),
        ]
        s3_workdir_bucket_policy_doc = iam.PolicyDocument(
            statements=s3_bucket_access_statements
        )

        # Create the compute environment
        instance_role = iam.Role(
            scope,
            "pavi-pipeline-compute-environment-instance-role",
            ...
            managed_policies=[
                ...
                iam.ManagedPolicy.from_managed_policy_name(
                    scope, "iam-ecr-read-policy", "ReadOnlyAccessECR"
                ),
            ],
            inline_policies={
        ...
        if not shared_logs_group:
            ...
            self.batch_log_group = cwl.LogGroup.from_log_group_name(
                scope, "pipeline-log-group", log_group_name=shared_logs_group
            )
        ...
        self.compute_environment = aws_batch.ManagedEc2EcsComputeEnvironment(
            scope=scope,
            id="pavi-pipeline-compute-environment-ecs",
            ...
            update_timeout=Duration.minutes(10),
        ...
        self.compute_environment.tags.set_tag(
            "Name",
            "PAVI pipeline execution worker",
            priority=None,
        ...
        self.job_queue = aws_batch.JobQueue(
            scope=scope, id="pavi-pipeline-job-queue", job_queue_name=jq_name
        )
        ...
        nf_aws_execution_policy = iam.ManagedPolicy(
            scope,
            "pavi-nextflow-aws-execution-policy",
