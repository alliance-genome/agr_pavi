...
def validate_strand_param(
    ...
    if value in STRAND_POS_CHOICES:
        ...
        raise click.BadParameter(
            f"Must be one of {STRAND_POS_CHOICES} for positive strand, or {STRAND_NEG_CHOICES} for negative strand."
        )
...
def process_seq_regions_param(
    ...
    try:
        ...
        if not isinstance(seq_regions, list):
            raise click.BadParameter(
                "Must be a valid list (JSON-array) of sequence regions to retrieve."
            )
        for index, region in enumerate(seq_regions):
            if isinstance(region, dict):
                if "start" not in region.keys():
                    raise click.BadParameter(
                        f"Region {region} does not have a 'start' property, which is a required property."
                    )
                if "end" not in region.keys():
                    raise click.BadParameter(
                        f"Region {region} does not have a 'end' property, which is a required property."
                    )
                if not isinstance(region["start"], int):
                    raise click.BadParameter(
                        f"'start' property of region {region} is not an integer. All positions must be integers."
                    )
                if not isinstance(region["end"], int):
                    raise click.BadParameter(
                        f"'end' property of region {region} is not an integer. All positions must be integers."
                    )
                if "frame" in region.keys():
                    valid_frame_types = get_args(SeqRegion.FRAME_TYPE)
                    if region["frame"] not in valid_frame_types:
                        raise click.BadParameter(
                            f"'frame' property of region {region} is not correctly typed. Value {region['frame']} must be one of {valid_frame_types}."
                        )
                else:
                    region["frame"] = None
                ...
                if re_match is not None:
                    region = dict(
                        start=int(re_match.group(1)),
                        end=int(re_match.group(2)),
                        frame=None,
                    )
                else:
                    raise click.BadParameter(
                        f"Region {region} of type string has invalid format. Region of type string must be formatted '`start`..`end`'"
                    )
            else:
                raise click.BadParameter(
                    f"Region {region} is not a valid type. All regions in seq_regions list must be valid dicts (JSON-objects) or strings."
                )
...
def process_variants_param(
    ...
    try:
        ...
        if not isinstance(variants_input, list):
            raise click.BadParameter(
                "Must be a valid list (JSON-array) of variant IDs to retrieve."
            )
        for index, variant in enumerate(variants_input):
            if not isinstance(variant, str):
                raise click.BadParameter(
                    f"Variant {variant} is not a valid string. All variants in variants list must be valid strings."
                )
            else:
...
def upload_to_s3(local_path: str, s3_prefix: str) -> None:
    ...
    result = subprocess.run(
        ["aws", "s3", "cp", local_path, s3_uri], capture_output=True, text=True
    )
...
def write_output(
    ...
    output_type: str,
    variants_flag: bool,
    alt_seq_name_suffix: str,
    ref_seq: Optional[str],
    alt_seq: Optional[str],
    ref_info: SeqInfo,
    alt_info: Optional[SeqInfo],
    sequence_output_file: str | None = None,
    ...
    with open(seq_info_output_file, "w") as output_file:
        ...
        output_file.write(
            jsonpickle.encode(indexed_seq_info, make_refs=False, unpicklable=False)
        )
...
@click.option(
    "--seq_strand",
    type=click.Choice(STRAND_POS_CHOICES + STRAND_NEG_CHOICES),
    default="+",
    callback=validate_strand_param,
...
@click.option(
    ...
    required=True,
    callback=process_seq_regions_param,
...
@click.option(
    ...
    type=click.UNPROCESSED,
    default="[]",
    callback=process_variants_param,
...
@click.option(
    "--output_type",
    type=click.Choice(["transcript", "protein"], case_sensitive=False),
    required=True,
...
def main(  # noqa: C901
    ...
    cds_seq_regions: List[SeqRegionDict],
    variant_ids: set[str],
    alt_seq_name_suffix: str,
    fasta_file_url: str,
    output_type: str,
    ...
    for variant_id in variant_ids:
        ...
        logger.debug(
            f"Variant info for {variant_id} fetched: {variant_info[variant_id]}"
        )
    ...
    fullRegion = TranslatedSeqRegion(
        exon_seq_regions=exon_seq_region_objs, cds_seq_regions=cds_seq_region_objs
    )

    logger.debug(
        f"full region: {fullRegion.seq_id}:{fullRegion.start}-{fullRegion.end}:{fullRegion.strand}"
    )
    ...
    if output_type == "transcript":
        ...
        if variant_info:
            ...
            try:
                seq_info = fullRegion.get_alt_sequence(
                    type="transcript",
                    unmasked=unmasked,
                    variants=list(variant_info.values()),
                )
            except Exception as e:  # pragma: no cover
                logger.error(
        ...
        if variant_info:
            ...
            try:
                ...
                error_msg = exception_description(e)
                alt_info = SeqInfo(error=error_msg, requested_variant_ids=list(variant_ids),
                                   non_coding_variants=non_coding_variants if non_coding_variants else None,
                                   species=species)
            else:
                alt_seq = seq_info.sequence
                alt_info = SeqInfo(embedded_variants=seq_info.embedded_variants,
                                   requested_variant_ids=list(variant_ids),
                                   non_coding_variants=non_coding_variants if non_coding_variants else None,
                                   species=species)

            if alt_seq == "":
                logger.error(
                    f"No ORF found for TranslatedSeqRegion {fullRegion} with variants embedded ({variant_ids})"
                )
    else:
        raise NotImplementedError(
            f"Output_type {output_type} is currently not implemented."
        )
    ...
    write_output(
        ...
        sequence_output_file=sequence_output_file,
        alt_seq_name_suffix=alt_seq_name_suffix,
        ref_seq=ref_seq,
        alt_seq=alt_seq,
        ref_info=ref_info,
        alt_info=alt_info,
        variants_flag=len(variant_info) > 0,
