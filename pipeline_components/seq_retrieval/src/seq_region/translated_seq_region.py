...
class TranslatedSeqRegion:
    ...
    def __init__(
        self, exon_seq_regions: List[SeqRegion], cds_seq_regions: List[SeqRegion] = []
    ):
        """
        ...
        self.start: int = min(
            map(lambda seq_region: seq_region.start, exon_seq_regions)
        )
        self.end: int = max(map(lambda seq_region: seq_region.end, exon_seq_regions))
        self.seq_length: int = sum(
            map(lambda seq_region: seq_region.seq_length, exon_seq_regions)
        )  # TODO: re-evaluate (coding vs complete)

        # Ensure one strand
        strands: Set[str | None] = set(
            [seq_region.strand for seq_region in (exon_seq_regions + cds_seq_regions)]
        )
        if len(strands) > 1:
            raise ValueError(
        ...
        seq_ids: Set[str] = set(
            map(
                lambda seq_region: seq_region.seq_id, exon_seq_regions + cds_seq_regions
            )
        )
        if len(seq_ids) > 1:
            raise ValueError(
        ...
        fasta_file_paths: Set[str] = set(
            map(
                lambda seq_region: seq_region.fasta_file_path,
                exon_seq_regions + cds_seq_regions,
            )
        )
        ...
        if len(cds_seq_regions) > 0:
            ...
            for seq_region in cds_seq_regions:
                if seq_region.frame is None:
                    raise ValueError(
                        f"undefined frame property found in seq region {seq_region}."
    ...
    def fetch_seq(
        self, type: Literal["transcript", "coding"], recursive_fetch: bool = False
    ) -> str:
        """
        ...
        match type:
            case "transcript":
                fetched_seq = self.exon_seq_region.fetch_seq(
            ...
            case "coding":
                if self.coding_seq_region:
                    ...
                    orfs = find_orfs(
                        dna_sequence, self.codon_table, return_type="longest"
                    )
                    ...
                    if len(orfs) > 0:
                        ...
                        self.coding_seq_region = self.exon_seq_region.sub_region(
                            rel_start=orf["seq_start"], rel_end=orf["seq_end"]
                        )
            ...
            case _:
                raise ValueError(
                    f"type {type} not implemented yet in TranslatedSeqRegion.fetch_seq method."
                )
    ...
    def get_sequence(
        self,
        type: Literal["transcript", "coding", "protein"],
        unmasked: bool = False,
        autofetch: bool = True,
    ) -> str:
        """
        ...
        match type:
            case "transcript":
                seq = self.exon_seq_region.get_sequence(
                    unmasked=unmasked, autofetch=autofetch
                )
            case "coding":
                if self.coding_dna_sequence is None:
                    if autofetch:
                        try:
                            ...
                            msg = (
                                "Unkown error occured while retrieving coding sequence."
                            )
                            logger.error(msg)
                            raise e
                    else:
                ...
                if self.protein_sequence is None:
                    if autofetch:
                        try:
                            ...
                            e.add_note(
                                "Translation error occured during coding sequence translation."
                            )
                            raise e
                        except Exception as e:  # pragma: no cover
                            e.add_note(
                                "Unexpected error occured during coding sequence translation."
                            )
                            raise e
                    else:
            ...
            case _:
                raise ValueError(
                    f"type {type} not implemented yet in MultiPartSeqRegion.set_multipart_sequence method."
                )
    ...
    def get_alt_sequence(
        ...
        match type:
            case "transcript":
                try:
                    alt_seq_info = self.exon_seq_region.get_alt_sequence(
                        unmasked=unmasked, variants=variants, autofetch=autofetch
                    )
                except Exception as e:  # pragma: no cover
            ...
            case "coding":
                ...
                try:
                    ...
                    msg = (
                        "No open reading frames found while retrieving coding sequence."
                    )
                    e.add_note(msg)
                    logger.warning(msg)
                    raise e
                ...
                alt_coding_seq_info = self.coding_seq_region.get_alt_sequence(
                    unmasked=unmasked,
                    variants=variants,
                    autofetch=autofetch,
                    inframe_only=True,
                ...
                if ref_start_codon != alt_start_codon:
                    err_msg = (
                        "Reference start codon is different from alternative start codon. "
                ...
                new_orfs = find_orfs(
                    dna_sequence=alt_coding_seq_info.sequence,
                ...
                if len(new_orfs) > 0:
                    ...
                    coding_alt_embedded_variants = (
                        SeqEmbeddedVariantsList.trimmed_on_rel_positions(
                            alt_coding_seq_info.embedded_variants,
                            trim_end=new_orfs[0]["seq_end"],
                        )
                    )
                    ...
                    logger.info(
                        "Stop codon not found in alternative coding sequence. "
                    ...
                    logger.debug(
                        "Original coding seq region: %s", self.coding_seq_region
                    )
                    ...
                    if self.strand == "-":
                        ref_coding_region_rel_start = (
                            self.exon_seq_region.to_rel_position(
                                self.coding_seq_region.end
                            )
                            + frame_offset
                        )
                    else:
                        ref_coding_region_rel_start = (
                            self.exon_seq_region.to_rel_position(
                                self.coding_seq_region.start
                            )
                            + frame_offset
                        )

                    extended_coding_region = self.exon_seq_region.sub_region(
                        rel_start=ref_coding_region_rel_start,
                        rel_end=self.exon_seq_region.seq_length,
                    )

                    logger.debug(
                        "Extended coding seq region: %s", extended_coding_region
                    )

                    extended_region_alt_seq_info = (
                        extended_coding_region.get_alt_sequence(
                            unmasked=unmasked,
                            variants=variants,
                            autofetch=autofetch,
                            inframe_only=True,
                    ...
                    extended_region_alt_orfs = find_orfs(
                        dna_sequence=extended_region_alt_seq_info.sequence,
                        codon_table=self.codon_table,
                        force_start=1,
                    )
                    ...
                    if not len(extended_region_alt_orfs) > 0:
                        err_msg = (
                            "Stop codon lost and no alternative found in extended alternative coding sequence. "
                    ...
                    coding_alt_embedded_variants = (
                        SeqEmbeddedVariantsList.trimmed_on_rel_positions(
                            extended_region_alt_seq_info.embedded_variants,
                            trim_end=extended_region_alt_orfs[0]["seq_end"],
                        )
                    )

                alt_seq_info = AltSeqInfo(
                    sequence=coding_alt_seq,
                    embedded_variants=coding_alt_embedded_variants,
                )
            ...
            case "protein":
                ...
                if len(variants) > 0:
                    try:
                        alt_coding_seq_info = self.get_alt_sequence(
                            type="coding",
                            unmasked=unmasked,
                            variants=variants,
                            autofetch=autofetch,
                        ...
                        e.add_note(
                            "Cannot translate due to failures in alternative coding sequence retrieval."
                        )
                        raise e
                    ...
                    try:
                        protein_alt_seq = self.translate(
                            coding_sequence=alt_coding_seq_info.sequence
                        )
                    except TranslationException as e:  # pragma: no cover
                        e.add_note(
                            "Translation error occured during alternative coding sequence translation."
                        )
                        raise e
                    except Exception as e:  # pragma: no cover
                        e.add_note(
                            "Unexpected error occured during alternative coding sequence translation."
                        )
                        raise e
                    else:
                    ...
                    if self.protein_sequence is None:
                        if autofetch:
                            ...
                            msg = (
                                "Protein sequence not stored and autofetch is disabled."
                            )
                            logger.error(msg)
                ...
                alt_seq_info = AltSeqInfo(
                    sequence=protein_alt_seq,
                    embedded_variants=protein_alt_embedded_variants,
                )

            case _:
                raise ValueError(
                    f"type {type} not implemented yet in TranslatedSeqRegion.get_alt_sequence method."
                )

        return alt_seq_info

    def set_sequence(
        self, type: Literal["transcript", "coding", "protein"], sequence: str
    ) -> None:
        """
        ...
        match type:
            ...
            case _:
                raise ValueError(
                    f"type {type} not implemented yet in TranslatedSeqRegion.set_sequence method."
                )
    ...
    def translate(self, coding_sequence: Optional[str] = None) -> str:
        ...
        if coding_sequence is None:
            try:
                ...
                e.add_note(
                    "Cannot translate due to failures in coding sequence retrieval."
                )
                raise e
            except Exception as e:  # pragma: no cover
...
def find_orfs(
    ...
    for frameshift in range(0, CODON_SIZE):
        codons[frameshift] = [
            unmasked_dna_sequence[i : i + CODON_SIZE]
            for i in range(frameshift, len(unmasked_dna_sequence), CODON_SIZE)
        ]
    ...
    for frameshift in range(0, CODON_SIZE):
        ...
        for i, codon in enumerate(codons[frameshift]):
            ...
            if (
                ...
                and i == 0
                and codon not in codon_table.start_codons
            ):
                raise ValueError(
                    "find_orfs expects first codon to be a start codon when using force_start argument."
                )

            if codon in codon_table.stop_codons:
                if reading_frame_opened:
                    seq_start = (
                        frameshift + index_opened * CODON_SIZE + 1 + force_start_offset
                    )  # Relative (DNA) sequence start position (1-based)
                    seq_end = (
                        frameshift + (i + 1) * CODON_SIZE + force_start_offset
                    )  # Relative (DNA) sequence end position (1-based)
                    orf: CalculatedOrf = {
