...
if external_api_base_url:
    ...
    client = Client(
        base_url=external_api_base_url, follow_redirects=False, timeout=timeout
    )
else:
...
def test_success_pipeline_workflow() -> None:
    ...
    input_data: str
    with open(
        "../tests/resources/submit-workflow-success-API-payload.json", mode="r"
    ) as input_file:
    ...
    assert response.status_code == 200, (
        f"Result retrieval for {job_uuid} did not return success."
    )

    with open(
        "../tests/resources/submit-workflow-success-output.aln", mode="r"
...
def test_fail_pipeline_workflow() -> None:
    ...
    input_data: str
    with open(
        "../tests/resources/pipeline_failure_seq_regions.json", mode="r"
    ) as input_file:
        input_data = input_file.read()
