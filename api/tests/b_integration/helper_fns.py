...
def poll_job_progress(
    client: TestClient | Client, job_uuid: UUID, timeout: int = 600, interval: int = 30
) -> dict[str, str]:
    """
    ...
    while walltime < timeout:
        ...
        assert response.status_code == 200, (
            f"Polling status for {job_uuid} did not return success."
        )
    ...
    assert walltime < timeout, (
        f"API failed to report job completion for {job_uuid} before timeout."
    )
